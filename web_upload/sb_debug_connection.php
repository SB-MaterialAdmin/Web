...
echo '[+] SourceBans "DebugConnection()" запущен для сервера ' . $serverip . ':' . $serverport . '<br /><br />';

// Попытаемся установить соединение
echo '[+] Открываю UDP-сокет...<br />';
$sock = @fsockopen("udp://" . $serverip, $serverport, $errno, $errstr, 2);

$isBanned = false;

if(!$sock)
    echo '[-] Ошибка соединения. #' . $errno . ': ' . $errstr . '<br />';
else {
    echo '[+] UDP-соединение успешно установлено!<br />';

    stream_set_timeout($sock, 1);

    // Попытаемся получить информацию у сервера
    echo '[+] Записываю запрос в сокет...<br />';
    if(fwrite($sock, "\xFF\xFF\xFF\xFF\x54Source Engine Query\0") === false)
        echo '[-] Ошибка записи.<br />';
    else {
        echo '[+] Запрос успешно записан в сокет. (Это не означает, что с соединением всё в порядке.) Читаю ответ...<br />';
        $packet = fread($sock, 1480);

        if(empty($packet))
            echo '[-] Ошибка при получении информации о сервере. Не удаётся прочитать UDP-соединение. Порт заблокирован.<br />';
        else {
            if(substr($packet, 5, (strpos(substr($packet, 5), "\0")-1)) == "Banned by server") {
                printf('[-] Ответ получен, но веб-сервер заблокирован. Удалите блокировку с сервера (removeip %s), и повторите попытку.<br />', $_SERVER['SERVER_ADDR']);
                $isBanned = true;
            } else {
                $packet = substr($packet, 6);
                $hostname = substr($packet, 0, strpos($packet, "\0"));
                echo '[+] Ответ получен! Сервер: ' . $hostname . ' <br />';
            }
        }
    }
    fclose($sock);
...
echo '[+] Попытка установить TCP-соединение...<br />';
$sock = @fsockopen($serverip, $serverport, $errno, $errstr, 2);
if(!$sock)
    echo '[-] Ошибка соединения. #' . $errno . ': ' . $errstr . '<br />';
else
{
    echo '[+] TCP-соединение успешно установлено!<br />';
    if(empty($serverrcon))
        ...
        echo '[+] Пытаюсь записать в TCP-сокет и произвести авторизацию...<br />';

        if(fwrite($sock, $data, strlen($data)) === false)
            echo '[-] Ошибка записи.<br />';
        else {
            echo '[+] Запрос авторизации успешно записан. Читаю ответ...<br />';
            $size = fread($sock, 4);
            if(!$size)
                ...
                if(empty($ret) || (isset($ret['ID']) && $ret['ID'] == -1))
                    echo '[-] RCON-пароль задан неверный ;) Не пытайтесь и дальше производить попытки, иначе ваш веб-сервер "улетит" в бан.<br />';
                else
