...
';
	...
	if(isset($_POST['postd']) && $_POST['postd'])
	{
		...
		else
		{
			...
			{
				// Setup Structure DB
				ProcessQueriesFile($db, INCLUDES_PATH . "/struc.sql");
				
				// Setup Admin
				$stmt = $db->prepare('INSERT INTO `'.$_POST['prefix'].'_admins` (`user`, `authid`, `password`, `gid`, `email`, `extraflags`, `immunity`) VALUES (?, ?, ?, ?, ?, ?, ?);');
				$stmt->execute([$_POST['uname'], $_POST['steam'], sha1(sha1(SB_SALT . $_POST['pass1'])), -1, $_POST['email'], (1<<24), 100]);
