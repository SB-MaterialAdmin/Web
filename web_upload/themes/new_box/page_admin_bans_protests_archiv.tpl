...
<div class="card">
  <div class="card-header">
    <h2>Протесты банов
      <small>
        Общее количество: {$protest_count_archiv}
      </small>
    </h2>
      ...
      <div class="col-sm-12 p-l-0">
        <div class="col-sm-2 p-0">
          <select class="selectpicker " name="bulk_action" id="bulk_action" onchange="BulkEdit(this,'{$admin_postkey}');">
            ...
            {/if}
            {if $KRUZYA}
            <option value="C">Восстановить</option>
            {/if}
          </select>
        </div>
        <div class="col-sm-3 p-r-0 text-right" style="float:right;">
          <button class="btn bgm-bluegray waves-effect" onclick="window.location.href='index.php?p=admin&c=bans#^0~p'">Активные протесты</button>
        </div>
      </div>
    </div>&nbsp;
</div>
