<?php
	...
	if (!$userbank->HasAccess(ADMIN_OWNER)) {
		...
		if (!empty($_GET['o']) and !empty($_GET['id']) and !is_numeric($_GET['id'])) {
			...
			} else if (($_GET['o'] == "on") and !empty($_GET['o'])) {
			...
			} else if (($_GET['o'] == "off") and !empty($_GET['o'])) {
				$check_sys = $GLOBALS['db']->GetOne("SELECT enabled FROM `" . DB_PREFIX . "_menu` WHERE id = '".(int)$_GET['id']."'");
		...
		if (isset($_POST['Link'])) {
			if ($_POST['Link'] == "add") {
				...
				$add = $GLOBALS['db']->Execute($insert_menu_sql, $prepaire_array);

				$js_message = sprintf("Ссылка была успешно создана%s!", ($_POST[$on_act] == 1) ? " и добавлена в меню" : "");
				$js_generate = sprintf("setTimeout(function() { %s; }, 1350);", generateMsgBoxJS("Успех!", $js_message, "green", "", true));
				AddScriptWithReload($js_generate, "index.php?p=admin&c=menu");
			}
		}
