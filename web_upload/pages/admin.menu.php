	...
	if(!$userbank->HasAccess(ADMIN_OWNER))
		...
		if(!empty($_GET['o']) and !empty($_GET['id']) and !is_numeric($_GET['id'])){
			...
			if(($_GET['o'] == "del") and !empty($_GET['o'])){
					$check_sys = \MaterialAdmin\DataStorage::ADOdb()->GetOne("SELECT system FROM `" . DB_PREFIX . "_menu` WHERE id = '".(int)$_GET['id']."'");
					if($check_sys != "1"){
						$gg_check_sys = \MaterialAdmin\DataStorage::ADOdb()->Execute("DELETE FROM `" . DB_PREFIX . "_menu` WHERE id = '".(int)$_GET['id']."'");
						if($gg_check_sys)
				...
				$check_sys = \MaterialAdmin\DataStorage::ADOdb()->GetOne("SELECT enabled FROM `" . DB_PREFIX . "_menu` WHERE id = '".(int)$_GET['id']."'");
				if($check_sys == "0"){
					$gg_check_sys = \MaterialAdmin\DataStorage::ADOdb()->Execute("UPDATE `" . DB_PREFIX . "_menu` SET `enabled` = '1' WHERE id = '".(int)$_GET['id']."'");
					if($gg_check_sys)
				...
				$check_sys = \MaterialAdmin\DataStorage::ADOdb()->GetOne("SELECT enabled FROM `" . DB_PREFIX . "_menu` WHERE id = '".(int)$_GET['id']."'");
				if($check_sys == "1"){
					$gg_check_sys = \MaterialAdmin\DataStorage::ADOdb()->Execute("UPDATE `" . DB_PREFIX . "_menu` SET `enabled` = '0' WHERE id = '".(int)$_GET['id']."'");
					if($gg_check_sys)
		...
		{ 
			if ($_POST['Link'] == "add"){
				...
				$add = \MaterialAdmin\DataStorage::ADOdb()->Execute("INSERT INTO `" . DB_PREFIX . "_menu` (`text`, `description`, `url`, `system`, `enabled`, `priority`, `newtab`) VALUES (?, ?, ?, 0, ?, ?, ?);", array($_POST['names_link'], $_POST['des_link'], $_POST['url_link'], $on_act, $_POST['priora_link'], (($_POST['onNewTab']=="on")?"1":"0"))); 
		...
		$list_menus = \MaterialAdmin\DataStorage::ADOdb()->GetAll("SELECT * FROM ".DB_PREFIX."_menu ORDER BY `priority` DESC");
