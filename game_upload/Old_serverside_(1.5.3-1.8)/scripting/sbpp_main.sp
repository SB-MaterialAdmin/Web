...
#endif
{
	...
	CreateNative("SourceBans_GetAdminExpire", Native_GetAdminExpire);
...
public Action:CommandBanIp(client, args)
{
	...
	if (!client)
	{
		...
		GetSID(client, adminAuth, sizeof(adminAuth));
	}
...
public GotDatabase(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if (loadAdmins && enableAdmins)
	{
		...
		if (serverID == -1)
		{
			FormatEx(query, 1024, "SELECT authid, srv_password, (SELECT name FROM %s_srvgroups WHERE name = srv_group AND flags != '') AS srv_group, srv_flags, user, immunity, expired  \
...
public VerifyInsert(Handle:owner, Handle:hndl, const String:error[], any:dataPack)
{
	...
	{
		decl String:szLength[64], String:szCreated[128], String:buffer[512], String:szAdminName[64];
		if(!time)
			Format(szLength, sizeof(szLength), "%T", "Permanent", client);
		else
		{
			new days = time / (60 * 24);
			new hours = (time - (days * (60 * 24))) / 60;
			new minutes = time % 60;
			new len = 0;
		...
		FormatTime(szCreated, sizeof(szCreated), NULL_STRING, GetTime());
		GetClientName(admin, szAdminName, sizeof(szAdminName));
		Format(buffer, sizeof(buffer), "%T", "Banned Admin", client, szAdminName, Reason, szCreated, szLength, WebsiteAddress);
...
public AdminsDone(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	while (SQL_MoreRows(hndl))
	{
		...
		ReplaceString(name, sizeof(name), "/", "_", false);
...
public Action:ClientRecheck(Handle:timer, any:client)
{
	...
	if (!PlayerStatus[client] && IsClientConnected(client) && GetSID(client, Authid, sizeof(Authid)))
	{
...
public OnClientPostAdminCheck(client)
{
	...
	if(admin != INVALID_ADMIN_ID)
	{
		...
		CreateTimer(15.0, Timer_Notify, hPack);
	}
}

public Action:Timer_Notify(Handle:timer, Handle:hPack)
{
	ResetPack(hPack);
	...
	else if(expire > 0)
	{
		...
		new len = 0;
		if(days) len += Format(szTimeStr[len], sizeof(szTimeStr) - len, "%d %T", days, "Days", client);
		if(hours) len += Format(szTimeStr[len], sizeof(szTimeStr) - len, "%s%d %T", days ? " " : "", hours, "Hours", client);
		if(minutes) len += Format(szTimeStr[len], sizeof(szTimeStr) - len, "%s%d %T", ((days && !hours) || hours) ? " " : "", minutes, "Minutes", client);
	}
	else
		return Plugin_Stop;
	
	PrintToChat(client, "[SM] %t", "Admin Expire", szTimeStr);
...
public bool:CreateBan(client, target, time, String:reason[])
{
	...
	if (!admin)
	{
		...
		GetSID(admin, adminAuth, sizeof(adminAuth));
	}
	...
	if (!GetSID(target, auth, sizeof(auth)))
		return false;
...
stock PrepareBan(client, target, time, String:reason[], size)
{
	...
	if (!GetSID(target, authid, sizeof(authid)))
		return;
	GetClientName(target, name, sizeof(name));
	...
	if (CreateBan(client, target, time, reason))
	{
		...
		decl String:szLength[64], String:szCreated[128], String:szAdminName[64];
		if(!time)
			Format(szLength, sizeof(szLength), "%T", "Permanent", client);
		else
		{
		...
		Format(bannedSite, sizeof(bannedSite), "%T", "Banned Admin", target, szAdminName, reason, szCreated, szLength, WebsiteAddress);
...
stock AccountForLateLoading()
{
	...
	for (new i = 1; i <= GetMaxClients(); i++)
	{
		...
		if (IsClientInGame(i) && !IsFakeClient(i) && IsClientAuthorized(i) && GetSID(i, auth, sizeof(auth)))
		{
...
stock bool:GetSID(iClient, String:szBuffer[], iLength) {
#if SOURCEMOD_V_MAJOR >= 1 && SOURCEMOD_V_MINOR >= 6
	return GetClientAuthId(iClient, AuthId_Steam2, szBuffer, iLength);
#else
