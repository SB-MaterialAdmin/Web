...
public Action:FWBlock(args)
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{
			GetSID(i, clientAuth, sizeof(clientAuth));
...
stock CreateBlock(client, targetId = 0, length = -1, type, const String:sReason[] = "", const String:sArgs[] = "")
{
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		#if defined DEBUG
		decl String:auth[64];
		GetSID(target, auth, sizeof(auth));
	...
	if (target_count > 1)
	{
		...
		for (new i = 0; i < target_count; i++)
		{
			...
			if (IsClientInGame(target))
				GetSID(target, targetAuth, sizeof(targetAuth));
			else
				continue;
...
public Native_SetClientMute(Handle:hPlugin, numParams)
{
    ...
    new bool:muteState = bool:GetNativeCell(2);
    ...
    new bool:bSaveToDB = bool:GetNativeCell(4);
