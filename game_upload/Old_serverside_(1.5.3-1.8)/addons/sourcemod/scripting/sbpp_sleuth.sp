...
public OnClientPostAdminCheck(client)
{
	if (CanUseSourcebans && !IsFakeClient(client))
	{
		...
		GetSID(client, steamid, sizeof(steamid));
		...
		if (FindStringInArray(g_hAllowedArray, steamid) == -1)
		{
			...
			FormatEx(query, sizeof(query), "SELECT * FROM %s_bans WHERE ip='%s' AND RemoveType IS NULL AND (ends > %d OR length = 0)", Prefix, IP, GetConVarInt(g_cVar_bantype) == 0 ? GetTime() : 0);
