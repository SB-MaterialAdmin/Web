...
{
	name = "SourceBans++ TR", 
	author = "SourceBans Development Team, Sarabveer(VEER™), NoTiCE", 
	description = "Advanced ban management for the Source engine", 
	version = SBR_VERSION, 
	url = "http://mmcs.pro/sb_time_reason.zip"
};
...
public Action:CommandBan(client, args)
{
	...
	if (!PlayerStatus[target])
	{
		...
		ReplyToCommand(admin, "[SM] %t", "Ban Not Verified");
		return Plugin_Handled;
	}
	...
	if (loadAdmins && enableAdmins)
	{
		...
		if (serverID == -1)
		{
			FormatEx(query, 1024, "SELECT authid, srv_password, (SELECT name FROM %s_srvgroups WHERE name = srv_group AND flags != '') AS srv_group, srv_flags, user, immunity, expired  \
						...
						WHERE %s (expired > UNIX_TIMESTAMP() OR expired = 0 OR expired = NULL) AND \
...
{
	new admin = ReadPackCell(dataPack); // admin index
	new client = ReadPackCell(dataPack);
	ReadPackCell(dataPack); // admin userid
	ReadPackCell(dataPack); // target userid
	...
	{
		decl String:szLength[64], String:szCreated[128], String:buffer2[512], String:szAdminName[64];
		if(!time)
			Format(szLength, sizeof(szLength), "%T", "Permanent", client);
		else
		{
		...
		FormatTime(szCreated, sizeof(szCreated), NULL_STRING, GetTime());
		GetClientName(admin, szAdminName, sizeof(szAdminName));
		Format(buffer2, sizeof(buffer2), "%T", "Banned Admin", client, szAdminName, reason, szCreated, szLength, WebsiteAddress);
		KickClient(client, buffer2);
	}
...
stock AccountForLateLoading()
{
	...
	for (new i = 1; i <= GetMaxClients(); i++)
	{
		...
		if (IsClientInGame(i) && !IsFakeClient(i) && IsClientAuthorized(i) && GetClientAuthString(i, auth, sizeof(auth)))
		{
